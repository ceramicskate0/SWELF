...
namespace SWELF
{
    public class EventLogEntry
    {
        ...
        public void GET_IP_FromLogFile()
        {
            ...
            {
                if (Settings.IP_RegX.IsMatch(line) && line.Contains('.') && line.Contains('\\') == false && string.IsNullOrEmpty(line)==false)
                {
                    Settings.IP_List_EVT_Logs.Add(line);
                }
                //TODO: if sysmon log get hostname add +(HOSTNAME) ie sysmon log line destinationhostname: HOSTNAME
            }
        }
