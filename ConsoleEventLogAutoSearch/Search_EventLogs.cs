        ...
        public Queue<EventLog_Entry> Search_EventLog(string Current_EventLog)
        {
            ...
            for (int x = 0; x < Settings.Search_Terms_Unparsed.Count; ++x)
            {
                try
                {
                    ...
                    {
                        Errors.Log_Error("Search()", "Value=" + Settings.Search_Terms_Unparsed.ElementAt(x-1) + ". Check syntax and data input names. Command to Long for input Format see docs.", Errors.LogSeverity.Warning);
                    }
                    else
                    {
