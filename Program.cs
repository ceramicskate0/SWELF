...
namespace SWELF
{
    public class Program
    {
            ...
            try
            {
                for (int x = 0; x < Settings.Plugin_Search_Terms_Unparsed.Count; ++x)
                {
                    ...
                    if (PSLog.EventData.ToLower().Contains(Settings.Plugin_Search_Terms_Unparsed.ElementAt(x).Split(spliter).ElementAt(1).ToLower()))
                    {
                        ...
                        try
                        {
                            HostEventLogAgent_Eventlog.WRITE_EventLog_From_SWELF_Search(Settings.SWELF_Events_Of_Interest_Matching_EventLogs.ElementAt(0));
        ...
        {
            //TODO IF files exists read in and dedup whats in memory
            //Do file size limit check if over size delete old data and replace with only new
            if (Settings.AppConfig_File_Args.ContainsKey("output_hashs"))
            {
                if (File.Exists(Settings.Hashs_File))
                {
                    Settings.Hashs_From_EVT_Logs.AddRange(Settings.READ_File_In(Settings.Hashs_File).Distinct().ToList());
                    Settings.Hashs_From_EVT_Logs = Settings.Hashs_From_EVT_Logs.Distinct().ToList();
                }
                Output_File.Write_IP_Output(Settings.Hashs_From_EVT_Logs.Distinct().ToList());
            }
            if (Settings.AppConfig_File_Args.ContainsKey("output_ips"))
            {
                if (File.Exists(Settings.IPs_File))
                {
                    Settings.IP_List_EVT_Logs.AddRange(Settings.READ_File_In(Settings.IPs_File).Distinct().ToList());
                    Settings.IP_List_EVT_Logs = Settings.IP_List_EVT_Logs.Distinct().ToList();
                }
                Output_File.Write_Hash_Output(Settings.IP_List_EVT_Logs.Distinct().ToList());
            }
        }
        }
}
