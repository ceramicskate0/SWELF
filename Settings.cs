...
CommentCharConfigs + @"log_collector: Must Be IPV4 : {Port num if not 514}{514 is default}
log_collector" + SplitChar_ConfigVariableEquals[0] + @"127.0.0.1" + 
CommentCharConfigs + @"output_format: syslogxml,syslog,xml,data,keyvalue
output_format" + SplitChar_ConfigVariableEquals[0] + @"keyvalue"+
CommentCharConfigs + @"log_level: verbose,informataion,warning,critical,failureaudit" +
"log_level" + SplitChar_ConfigVariableEquals[0] + @"critical"+
CommentCharConfigs + @"output_ips"+
CommentCharConfigs + @"ouput_hashs";
            return log;
        }
...
'""");
        ...
        {
           return Dns.GetHostEntry(IPAddress.Parse(IP)).ToString();
        }

        public static List<string> READ_File_In(string FilePath)
        {
