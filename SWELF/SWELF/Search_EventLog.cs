...
namespace SWELF
{
    internal class Search_EventLog
    {
        ...
        private List<EventLog_Entry> SWITCH_to_Parse_SearchRule(string[] Search_String_Parsed, string Current_EventLog, string SearchRule)
        {
            try
            {
                string[] Search_String_Parsed_Empty_Removed = SearchRule.Split(Settings.SplitChar_SearchCommandSplit, StringSplitOptions.RemoveEmptyEntries).ToArray();

                switch (Search_String_Parsed_Empty_Removed.Length)
                {
                    case 1://search term and event id
                        {//Term,log,id
                            if (Read_In_EventLogs_From_WindowsAPI.Count > 0)
                            {
                                ...
                                {
                                   return SEARCH_Everything(Search_String_Parsed[0]);
                                }
                                ...
                                else
                                {
                                    try
                                    {
                                        if (Search_String_Parsed[1].ToLower() == Current_EventLog.ToLower())
                                        {
                                            ...
                                            return SEARCH_Everything(UnparsedTerm);
                                        }
            ...
            catch (Exception e)
            {
                ...
                Error_Operation.Log_Error("SEARCH_FindTerms()", "Unable to finish search and index of logs for event log named '" + SearchRule + "' with Current_EventLog '"+Current_EventLog +"' "+"Search term was '"+ Data+"' error message is '"+ e.Message.ToString()+"'", e.StackTrace.ToString(), Error_Operation.LogSeverity.Critical);
                return new List<EventLog_Entry>();
            }
        }
        ...
        */
            ALL_Logs_That_Matched_Search_This_Event_Log.AddRange(SWITCH_to_Parse_SearchRule(Search_String_Parsed, Current_EventLog, Settings.Search_Rules_Unparsed.ElementAt(x)));
        }
        ...
        private List<EventLog_Entry> SEARCH_Everything(string SearchRule)
        {
           IList<EventLog_Entry> results = Read_In_EventLogs_From_WindowsAPI.Where(s => s.EventData.Contains(SearchRule) || s.EventID.ToString().Equals(SearchRule)).ToList();
        ...
        {
            if (EventLogName.ToLower() == Read_In_EventLogs_From_WindowsAPI.Peek().LogName.ToLower())
            {
                int Limit = 10;
                ...
                if (SearchTerms.Length > 1)
                {
                    for (int x = 0; x < SearchTerms.Length; ++x)
                    {
                        ...
                        else
                        {
                            ...
                            Read_In_EventLogs_From_WindowsAPI = new Queue<EventLog_Entry>(SWITCH_to_Parse_SearchRule(searchthis, EventLogName, SearchRule));
                            ...
                            {
                              Read_In_EventLogs_From_WindowsAPI = Temp_Storage;//make all finds be the next thing searched for all searchs
                                return new List<EventLog_Entry>();
                            }
                            else
                            {
